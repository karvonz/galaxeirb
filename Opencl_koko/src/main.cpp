...
*/
	...
	float4 p_out[N_PARTICULE];
	float4 v[N_PARTICULE];
	...
	{
		p[i]=make_float4(list[i].px,list[i].py,list[i].pz, 0.0f);
		v[i]=make_float4(list[i].vx,list[i].vy,list[i].vz, list[i].m);
	}
	
printf("Nombre de particule = %d\n ", tailleFichier);
	

	float4 *d_p,*d_p_out, *d_acceleration;
	
	bool boole=false;

	CUDA_MALLOC((void **)&d_p, sizeof(float4)*tailleFichier);
	CUDA_MALLOC((void **)&d_p_out, sizeof(float4)*tailleFichier);
	CUDA_MALLOC((void **)&d_acceleration, sizeof(float4)*tailleFichier);

	


CUDA_MEMCPY( d_p, p, sizeof(float4)*tailleFichier, cudaMemcpyHostToDevice);
CUDA_MEMCPY( d_acceleration, v, sizeof(float4)*tailleFichier, cudaMemcpyHostToDevice);
	...
	while ( !done ) {
		...
		DrawGalaxy(p_out, tailleFichier);
		if (boole)
			CUDA_MEMCPY( d_p, p_out, sizeof(float4)*tailleFichier, cudaMemcpyHostToDevice);
		else
			boole=true;	

		updateGalaxy( numBlocks, 256, d_p, d_acceleration, d_p_out );
		//printf("before");
		...
		CUDA_MEMCPYBACK( p_out, d_p_out, sizeof(float4)*tailleFichier, cudaMemcpyDeviceToHost);
