...
void UpdateVertices (){
	...
	for(i=0 ; i<NB_PARTICLES*4 ; i=i+4  )
	{
		...
		vVertices[i]=pout[j].x/64.0f;
		vVertices[i+1]=pout[j].y/64.0f;
		vVertices[i+2]=pout[j].z/64.0f;
		j++;
	}
}
...
////////////////////////////////
	...
	pout = (cl_float4*)clEnqueueMapBuffer(commandQueue, memoryObjects[1], CL_TRUE, CL_MAP_READ, 0, bufferSize, 0, NULL, NULL, &errorNumber);
...
if (!checkSuccess(clEnqueueUnmapMemObject(commandQueue, memoryObjects[1], pout, 0, NULL, NULL)))
    {
       cleanUpOpenCL(context, commandQueue, program, kernel, memoryObjects, numberOfMemoryObjects);
...
////////////////////////////////
	...
	memoryObjects[0] = clCreateBuffer(context,  CL_MEM_READ_WRITE  | CL_MEM_ALLOC_HOST_PTR, bufferSize*2, NULL, &errorNumber);
	...
	popencl = (cl_float4*)clEnqueueMapBuffer(commandQueue, memoryObjects[0], CL_TRUE, CL_MAP_WRITE, 0, bufferSize*2, 0, NULL, NULL, &errorNumber);
	...
	pout = (cl_float4*)clEnqueueMapBuffer(commandQueue, memoryObjects[1], CL_TRUE, CL_MAP_WRITE, 0, bufferSize, 0, NULL, NULL, &errorNumber);
...
//mtn j'écris dans la mémoire en utilisant les pointeurs sur la shared memory
	for (int i = 0; i < NB_PARTICLES; i++)
	{
		...
		popencl[i+NB_PARTICLES].x = list[i].vx;
		popencl[i+NB_PARTICLES].y = list[i].vy;
		popencl[i+NB_PARTICLES].z = list[i].vz;
		popencl[i+NB_PARTICLES].w = list[i].m;
