...
inline void DrawGalaxy(float3 *p, int size)
{		glPointSize(1.0f);
glBegin( GL_POINTS );
	int i;
	/*for (i=0; i <16384; i++)
...
printf("Nombre de particule = %d\n ", tailleFichier);
	...
	float *d_m;
	float3 *d_p,*d_p_out, *d_acceleration;
	
	bool boole=false;

	CUDA_MALLOC((void **)&d_p, sizeof(float3)*tailleFichier);
	CUDA_MALLOC((void **)&d_p_out, sizeof(float3)*tailleFichier);
...
CUDA_MEMCPY( d_acceleration, v, sizeof(float3)*tailleFichier, cudaMemcpyHostToDevice);
...
if (boole)
	CUDA_MEMCPY( d_p, p_out, sizeof(float3)*tailleFichier, cudaMemcpyHostToDevice);
else
	boole=true;	
		//UpdateGalaxy(list, 
		updateGalaxy( numBlocks, 1024, d_m, d_p, d_acceleration, d_p_out );
		//printf("before");
	...
	CUDA_MEMCPYBACK( p_out, d_p_out, sizeof(float3)*tailleFichier, cudaMemcpyDeviceToHost);
