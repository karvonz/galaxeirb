...
cudaError_t cudaStatus;
	...
	float acceleration[N_PARTICULE][3]={0.0f};
	float (*d_acceleration)[3] = NULL;
	particule *d_list;

	CUDA_MALLOC((void **)&d_list, sizeof(particule)*N_PARTICULE);
	CUDA_MALLOC((void **)&d_acceleration, sizeof(float)*N_PARTICULE*3);
	...
	int numBlocks = ( N_PARTICULE + ( N_THREADS - 1 ) ) / N_THREADS;
		...
		CUDA_MEMCPY( d_list, list, sizeof(particule)*N_PARTICULE, cudaMemcpyHostToDevice);
		//UpdateGalaxy(list, 
		updateGalaxy( numBlocks, 1024, d_list, d_acceleration );
		//printf("before");
	...
	if ( cudaStatus != cudaSuccess ) {
		...
		CUDA_MEMCPYBACK( acceleration, d_acceleration, sizeof(float)*3*N_PARTICULE, cudaMemcpyDeviceToHost);
	
	//printf("acceleration vx=%f\n", acceleration[0][0]);
		
	//#pragma omp parallel for
	for (i=0; i<N_PARTICULE; i++){
		list[i].vx+=acceleration[i][0]*TO* M;
		list[i].vy+=acceleration[i][1]*TO* M;
		list[i].vz+=acceleration[i][2]*TO* M;
